...
export default function AskNameForTwoPlayer() {
  ...
  const handleSubmit = () => {
    // localStorage.setItem("offline/player1", detail.player1);
    // localStorage.setItem("offline/player2", detail.player2);
    dispatch(setPlayers(detail));
  };
  return (
    <div className='menuContainer grid place-content-center h-screen'>
      <div className='menu w-[350px] xsm:w-[270px] shadow-xl p-4 py-8 xsm:p-0 xsm:shadow-none'>
        <div className='player1 mb-6'>
          <label htmlFor='player1' className='font-bold text-lg'>
            Player 1
          </label>
          <br />
          <input
